...
public abstract class WebUtils {
	...
	private static HttpURLConnection getConnection(URL url, String method, String ctype) throws IOException {
		...
		conn.setRequestProperty("secret", "984d75d6866248a5ae31e346c9e4f0b8");
		conn.setRequestProperty("certificateId", "127.0.0.1");
		return conn;
	}
	/**
	 * 
		...
		if (!CommonHelper.isEmpty(ctype)) {
			...
			for (String param : params) {
				...
				if (param.startsWith("charset")) {
					...
					if (pair.length == 2) {
						if (!CommonHelper.isEmpty(pair[1])) {
							charset = pair[1].trim();
						}
					}
					break;
